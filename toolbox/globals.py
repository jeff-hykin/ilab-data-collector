...
# load up
INFO = json.loads(FS.read("./settings/info.json"))
    ...
    for each_key, each_value in dict_mapping.items():
        ...
        if type(each_key) is str:
            ...
            if type(each_value) is str:
                *folders, name, ext = FS.path_pieces(each_value)
                ...
                if folders[0] != "/":
                    if folders[0] == '.' or folders[0] == './':
                        _, *folders = folders
                    resulting_paths[each_key] = Path(FS.absolute_path(FS.join(source, each_value)))
                else:
                    resulting_paths[each_key] = Path(each_value)
    
    return resulting_paths

PATHS = process_paths_mapping(FS.pwd(), INFO["paths"])
